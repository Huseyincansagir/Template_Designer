...
function bindingKey(binding: ShortcutBinding, platform: string): string {
  return `${platform}:${binding.mod ? "mod+" : ""}${binding.shift ? "shift+" : ""}${binding.key.toLowerCase()}`;
}
...
export function shortcutDisplay(descriptor: ShortcutDescriptor): string {
  const { mod, shift, key } = descriptor.binding;
  const parts: string[] = [];
...
/** The canonical CONFIRMED table (UI §19). R (90° rotation) is excluded: free rotation is a future transform contract. */
