...
import { createEditorApplication, defaultWidgetName, uniqueDefaultName, type AssetDraft, type MutationResult } from "../Core/editor-application";
...
function coerceRuntimeInput(raw: string, type: RuntimeValueType): PrimitiveValue | null {
  ...
  if (type === "string" || type === "enum") return raw.normalize("NFC");
  return raw;
}
...
export function mappedFloorDisplay(mapping: { entries: readonly { firmwareValue: unknown; displayValue: string }[] } | undefined, sourceValue: unknown): string | undefined {
  ...
  return mapping.entries.find((entry) => String(entry.firmwareValue).normalize("NFC") === needle)?.displayValue;
}
...
export function App({ profileRegistry }: { profileRegistry: DeviceProfileRegistry }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      for (const entry of fetched) {
        if (entry) incoming[entry.assetId] = entry.preview;
      }
  ...
  const addScene = (requestedName?: string): boolean => {
    ...
    const name = uniqueDefaultName(requestedName?.trim() || "New Scene", rotation?.scenes.map((scene) => scene.name) ?? []);
  ...
  const defaultWidgetBox = (widgetType: string): { width: number; height: number } => {
    if (widgetType === "digit") return { width: 280, height: 180 };
    if (widgetType === "direction") return { width: 160, height: 120 };
    if (widgetType === "warning") return { width: 360, height: 72 };
    if (widgetType === "media") return { width: 240, height: 160 };
    return { width: 200, height: 48 };
  };

  const enterPlaceMode = (widgetType: string): boolean => {
    if (blockedInPreview("Add Widget")) return false;
    if (!activeScene?.id || !activeProfile) {
    ...
    setPlaceWidgetType(widgetType);
    logAction(`Place ${defaultWidgetName(widgetType)}: click the canvas · Esc exits`, "EVENT");
    return true;
  };

  const addDisplayKit = (): boolean => {
    if (blockedInPreview("Add Widget")) return false;
    if (!activeScene?.id || !activeRotation || !activeProfile) {
      logAction("Display kit blocked: select an active Scene first", "WARN");
      return false;
    }
    const width = activeRotation.width;
    const height = activeRotation.height;
    const types = new Set(activeProfile.supportedWidgetTypes);
    const specs: { widgetType: string; geometry: Geometry; name: string }[] = [];
    if (types.has("media")) specs.push({ widgetType: "media", name: "Background", geometry: { x: 0, y: 0, width, height } });
    if (types.has("digit")) specs.push({ widgetType: "digit", name: "Floor", geometry: { x: Math.round(width * 0.2), y: Math.round(height * 0.28), width: Math.round(width * 0.6), height: Math.round(height * 0.22) } });
    if (types.has("direction")) specs.push({ widgetType: "direction", name: "Direction", geometry: { x: Math.round(width * 0.35), y: Math.round(height * 0.16), width: Math.round(width * 0.3), height: Math.round(height * 0.1) } });
    if (types.has("warning")) specs.push({ widgetType: "warning", name: "Warning", geometry: { x: Math.round(width * 0.1), y: Math.round(height * 0.54), width: Math.round(width * 0.8), height: Math.round(height * 0.1) } });
    if (types.has("text")) specs.push({ widgetType: "text", name: "Caption", geometry: { x: Math.round(width * 0.1), y: Math.round(height * 0.82), width: Math.round(width * 0.8), height: Math.round(height * 0.08) } });
    if (!specs.length) {
      logAction("Display kit: the active DeviceProfile declares no widget types", "WARN");
      return false;
    }
    const result = editorApplication.addWidgets(activeScene.id, specs);
    if (!result.changed) {
      logAction("Display kit failed", "WARN");
      return false;
    }
    const createdIds = result.createdIds ?? [];
    if (createdIds[0]) {
      setExpandedNodes((current) => ({ ...current, [activeScene.id]: true, ...(activeRotation ? { [activeRotation.id]: true } : {}) }));
      setSelectedIds([createdIds[0]]);
      setSelection({ id: createdIds[0], label: specs[0].name, kind: "widget", nodeType: specs[0].widgetType, detail: "Visible" });
    }
    logAction(`Display kit added · ${createdIds.length} widget(s)`, "EVENT");
    return true;
  };

  const addWidget = (widgetType: string, at?: CanvasPoint): boolean => {
    if (blockedInPreview("Add Widget")) return false;
    ...
    const { width, height } = defaultWidgetBox(widgetType);
    const cascade = (activeScene?.widgets.length ?? 0) % 8;
    const stepX = width + snapGridSize;
    const stepY = height + snapGridSize;
    const x = at
      ? Math.round((at.x - width / 2) / snapGridSize) * snapGridSize
      : Math.max(0, Math.round((((activeRotation.width - width) / 2) + cascade * stepX) / snapGridSize) * snapGridSize);
    const y = at
      ? Math.round((at.y - height / 2) / snapGridSize) * snapGridSize
      : Math.max(0, Math.round((((activeRotation.height - height) / 2) + cascade * stepY) / snapGridSize) * snapGridSize);
  ...
  const setWidgetContentValue = (key: string, value: unknown, target?: { sceneId: string; widgetId: string }): boolean => {
    ...
    return configureWidget({ content: Object.keys(nextContent).length ? nextContent : undefined }, `Set ${key}`, target);
  };
  ...
  const setWidgetStyleValue = (key: string, value: unknown, target?: { sceneId: string; widgetId: string }): boolean => {
    ...
    return configureWidget({ style: Object.keys(nextStyle).length ? nextStyle : undefined }, `Set ${key}`, target);
  };
  ...
  const setThemeResources = (themeId: string, assetIds: readonly string[]): boolean => {
    if (blockedInPreview("Theme resources")) return false;
  ...
  const adoptProfileVersion = (): boolean => {
    if (blockedInPreview("Adopt Profile Version")) return false;
  ...
  const enterDuplicateMode = (): boolean => {
    if (blockedInPreview("Duplicate Mode")) return false;
  ...
  const addBinding = (): boolean => {
    if (blockedInPreview("Add Binding")) return false;
  ...
  const setBindingPriority = (bindingId: string, priority: number): boolean => {
    if (blockedInPreview("Binding priority")) return false;
    const resolved = bindingModal ? resolveCanonicalNode(project, bindingModal.widgetId) : undefined;
    const sceneId = resolved?.scene?.id;
    const widget = resolved?.widget;
  ...
  const setBindingConditionMode = (bindingId: string, mode: ConditionMode): boolean => {
    if (blockedInPreview("Binding condition mode")) return false;
    const resolved = bindingModal ? resolveCanonicalNode(project, bindingModal.widgetId) : undefined;
    const sceneId = resolved?.scene?.id;
    const widget = resolved?.widget;
    if (!sceneId || !widget) return false;
  ...
  const removeBindingCondition = (bindingId: string, index: number): boolean => {
    if (blockedInPreview("Remove binding condition")) return false;
    const resolved = bindingModal ? resolveCanonicalNode(project, bindingModal.widgetId) : undefined;
    const sceneId = resolved?.scene?.id;
    const widget = resolved?.widget;
  ...
  const removeBinding = (bindingId: string): boolean => {
    if (blockedInPreview("Remove Binding")) return false;
    const resolved = bindingModal ? resolveCanonicalNode(project, bindingModal.widgetId) : undefined;
    const sceneId = resolved?.scene?.id;
    const widget = resolved?.widget;
    if (!sceneId || !widget) return false;
  ...
  const executeEditorDescriptor = (commandId: EditorCommandId) => {
    ...
    else if (commandId.startsWith("scene.add-widget:")) { enterPlaceMode(commandId.slice("scene.add-widget:".length)); setContextMenu(null); return; }
  ...
  const handleCanvasClick = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (placeWidgetType) {
      if (blockedInPreview("Add Widget")) return;
      addWidget(placeWidgetType, toCanvasPoint(event));
      return;
    }
    if (duplicateMode) {
      if (blockedInPreview("Duplicate place")) return;
      const sceneId = activeScene?.id;
  ...
  const handleGlobalKeyDown = (event: KeyboardEvent) => {
    ...
    if (event.key === "Escape") {
      event.preventDefault();
      if (placeWidgetType) {
        setPlaceWidgetType(null);
        logAction("Place mode exited", "EVENT");
        return;
      }
      if (duplicateMode) {
        setDuplicateMode(false);
  ...
  const menuItems: Record<MenuKey, MenuItem[]> = {
    ...
    Widget: [
      ...(activeProfile?.supportedWidgetTypes ?? []).map((widgetType) => ({ label: `Add ${defaultWidgetName(widgetType)} Widget`, disabled: !activeScene?.id, title: activeScene?.id ? "Click the canvas to place" : "Requires an active Scene", onClick: () => enterPlaceMode(widgetType) })),
      { label: "Add Display Kit", disabled: !activeScene?.id, title: activeScene?.id ? "Place a profile-driven set of widgets on this Scene" : "Requires an active Scene", onClick: addDisplayKit },
      ...
      <div className="editor-chrome" aria-label="Editor tools, rotation and scene navigation">
        <div className="editor-chrome-row">
          <div className="tool-group">
            ...
            <span className="tool-divider" />
            {(activeProfile?.supportedWidgetTypes ?? []).map((widgetType) => (
              <button
                type="button"
                key={widgetType}
                className={`studio-tool ${placeWidgetType === widgetType ? "active" : ""}`}
                disabled={viewMode === "preview" || !activeScene?.id}
                title={activeScene?.id ? `Place ${defaultWidgetName(widgetType)} · click the canvas` : "Requires an active Scene"}
                onClick={() => enterPlaceMode(widgetType)}
              >{widgetType === "digit" ? "88" : widgetType === "direction" ? "▲" : widgetType === "warning" ? "!" : widgetType === "media" ? "▣" : "T"} <span>{defaultWidgetName(widgetType)}</span></button>
            ))}
          </div>
          <span className="tool-divider" />
    ...
    if (widget.widgetType === "text" || widget.widgetType === "warning") {
      return (
        <section className="property-section">
          ...
          <div className="property-row property-row-edit"><span>{widget.widgetType === "text" ? "Text" : "Message"}</span><DraftTextField scope={`${widget.id}:text`} value={textValue} disabled={viewMode === "preview"} placeholder="Not set" ariaLabel={`${widget.widgetType} content text`} onCommit={(value) => setWidgetContentValue("text", value, contentTarget)} /></div>
      ...
      return (
        <section className="property-section">
          ...
          <div className="property-row property-row-edit">
            ...
            <select aria-label="Floor mapping" value={String(widget.content?.floorMappingId ?? "")} disabled={mappings.length === 0} title={mappings.length === 0 ? "This Theme Project declares no Floor Mapping" : undefined} onChange={(event) => setWidgetContentValue("floorMappingId", event.target.value, contentTarget)}>
          ...
          <>
                ...
                {slide.items.map((item, index) => {
                  ...
                  return (
                    <li key={item.id}>
                      ...
                      <DraftNumberField scope={`${widget.id}:seq:${item.id}`} value={String(item.duration)} disabled={viewMode === "preview"} min={0} max={3600} decimals={1} ariaLabel={`Entry ${index + 1} duration in seconds`} onCommit={(value) => updateSequenceItem(widget, item.id, { duration: Math.round(value * 10) / 10 })} />
                      <DraftNumberField scope={`${widget.id}:seqrepeat:${item.id}`} value={String(item.repeatCount ?? 0)} disabled={viewMode === "preview"} min={0} max={999} integer ariaLabel={`Entry ${index + 1} repeat count`} onCommit={(value) => updateSequenceItem(widget, item.id, { repeatCount: value > 0 ? value : undefined })} />
            ...
            {slide && (
              <>
                ...
                <div className="property-row property-row-edit"><span>Repeat Sequence</span><DraftNumberField scope={`${widget.id}:seqrepeat`} value={String(slide.repeatCount ?? 0)} disabled={viewMode === "preview"} min={0} max={999} integer ariaLabel="Media sequence repeat count" onCommit={(value) => configureWidget({ mediaSlide: { ...slide, repeatCount: value } }, "Set sequence repeat count")} /></div>
                ...
                <div className="property-row property-row-edit"><span>Sequence Volume</span><DraftNumberField scope={`${widget.id}:seqvolume`} value={String(slide.volume ?? 0)} disabled={viewMode === "preview"} min={0} max={100} integer ariaLabel="Media sequence volume 0 to 100" onCommit={(value) => configureWidget({ mediaSlide: { ...slide, volume: value > 0 ? Math.min(100, value) : undefined } }, "Set sequence volume")} /></div>
  ...
  const renderFloorMappingSection = (theme: ThemeProject) => {
    ...
    const commit = (next: readonly FloorMapping[], label: string) => {
      if (blockedInPreview("Floor mapping")) return false;
                ...
                <ul className="floor-entry-list">
                  {mapping.entries.map((entry, index) => (
                    <li key={`${mapping.id}-${index}`}>
                      <DraftTextField scope={`${mapping.id}:${index}:firmware`} value={String(entry.firmwareValue)} disabled={viewMode === "preview"} placeholder="firmware id" ariaLabel={`Firmware identifier ${index + 1}`} onCommit={(value) => patchEntry(mapping.id, index, { firmwareValue: value })} />
                      <span className="floor-arrow" aria-hidden="true">→</span>
                      <DraftTextField scope={`${mapping.id}:${index}:display`} value={entry.displayValue} disabled={viewMode === "preview"} placeholder="shown" ariaLabel={`Display value ${index + 1}`} onCommit={(value) => patchEntry(mapping.id, index, { displayValue: value })} />
  ...
  const renderProperties = () => {
    ...
    const selectedSceneWidgetsForEdit = selectedCanonical.map((current) => current.widget).filter((candidate): candidate is Widget => Boolean(candidate));
    const geometryEditable = viewMode !== "preview" && canvasPointer.mode === "idle" && selectedSceneWidgetsForEdit.length > 0 && selectedSceneWidgetsForEdit.every((current) => !current.locked);
    const layerEditable = geometryEditable;
    ...
    return (
      <>
        ...
        {selection && node ? <div className="properties-scroll">
          ...
          {widget && <>
            <section className="property-section"><div className="property-section-title">Widget</div>{multi ? <PropertyRow label="Widget Type" value={valueFor((current) => current.widget?.widgetType)} /> : <div className="property-row property-row-edit"><span>Widget Type</span><select aria-label="Widget type" value={widget.widgetType} disabled={viewMode === "preview"} onChange={(event) => changeWidgetType(event.target.value)}>{(activeProfile?.supportedWidgetTypes ?? []).map((widgetType) => <option key={widgetType} value={widgetType}>{defaultWidgetName(widgetType)}</option>)}{!(activeProfile?.supportedWidgetTypes ?? []).includes(widget.widgetType) && <option value={widget.widgetType}>{widget.widgetType} (unsupported)</option>}</select></div>}<div className="property-row property-row-edit"><span>Visible</span><input type="checkbox" aria-label="Widget visible" disabled={viewMode === "preview"} checked={multi ? selectedSceneWidgets.length > 0 && selectedSceneWidgets.every((current) => current.visible) : widget.visible} onChange={() => toggleWidgetProperty("visible")} /></div><div className="property-row property-row-edit"><span>Enabled</span><input type="checkbox" aria-label="Widget enabled" disabled={viewMode === "preview"} checked={multi ? selectedSceneWidgets.length > 0 && selectedSceneWidgets.every((current) => current.enabled) : widget.enabled} onChange={() => toggleWidgetProperty("enabled")} /></div><div className="property-row property-row-edit"><span>Geometry Lock</span><input type="checkbox" aria-label="Widget geometry lock" disabled={viewMode === "preview"} checked={multi ? selectedSceneWidgets.length > 0 && selectedSceneWidgets.every((current) => current.locked) : widget.locked} onChange={() => toggleWidgetProperty("locked")} /></div></section>
            <section className="property-section"><div className="property-section-title">Geometry / Layer</div><div className="geometry-editor"><GeometryField scope={`${draftScope}:x`} label="X" field="x" value={multi ? valueFor((current) => current.widget ? canonicalGeometry(current.widget).x : undefined) : canonicalGeometry(widget).x} multi={multi} disabled={!geometryEditable} min={0} max={geometryBound("x")} onCommit={commitSelectionGeometryField} /><GeometryField scope={`${draftScope}:y`} label="Y" field="y" value={multi ? valueFor((current) => current.widget ? canonicalGeometry(current.widget).y : undefined) : canonicalGeometry(widget).y} multi={multi} disabled={!geometryEditable} min={0} max={geometryBound("y")} onCommit={commitSelectionGeometryField} /><GeometryField scope={`${draftScope}:w`} label="W" field="width" value={multi ? valueFor((current) => current.widget ? canonicalGeometry(current.widget).width : undefined) : canonicalGeometry(widget).width} multi={multi} disabled={!geometryEditable} min={10} max={geometryBound("width")} onCommit={commitSelectionGeometryField} /><GeometryField scope={`${draftScope}:h`} label="H" field="height" value={multi ? valueFor((current) => current.widget ? canonicalGeometry(current.widget).height : undefined) : canonicalGeometry(widget).height} multi={multi} disabled={!geometryEditable} min={10} max={geometryBound("height")} onCommit={commitSelectionGeometryField} /></div><div className="property-row property-row-edit"><span>Z-order</span><DraftNumberField scope={`${draftScope}:z`} value={multi ? valueFor((current) => current.widget?.zIndex) : String(widget.zIndex)} disabled={!layerEditable} min={-100000} max={100000} ariaLabel="Widget z-order" onCommit={(value) => { if (blockedInPreview("Z-order")) return; const sceneId = activeScene?.id; if (!sceneId) return; const unlocked = selectedWidgetIds.filter((id) => !resolveCanonicalNode(project, id)?.widget?.locked); if (!unlocked.length) return; const updates = Object.fromEntries(unlocked.map((id) => [id, value])); const result = editorApplication.setWidgetZIndicesInScene(sceneId, updates); if (result.changed) logAction(`Set widget zIndex to ${value}`, "EVENT"); }} /></div></section>
            <section className="property-section"><div className="property-section-title">Presentation</div><PropertyRow label="Bindings" value={String(widget.bindings.length)} /><button type="button" className="property-inline-action" disabled={viewMode === "preview"} title={viewMode === "preview" ? "Preview Mode is inspection-only" : undefined} onClick={() => setBindingModal({ widgetId: widget.id })}>Open Binding Editor</button></section>
  ...
  const renderDeployment = () => {
    ...
    return (
      <div className="deployment-panel">
        ...
        <ul className="deployment-readiness" aria-label="Package readiness">
          <li>{activeProfile ? "Profile compatible" : "No DeviceProfile"}</li>
          <li>{(activeTheme?.rotations.length ?? 0) === 4 ? "Required rotations available (4/4)" : "Required rotations missing"}</li>
          <li>{validation.issues.some((issue) => issue.severity === "error" && issue.code.includes("ASSET")) ? "Asset / format errors" : "Media formats review"}</li>
          <li>{validation.valid ? "Runtime / bindings valid" : `${validation.issues.filter((issue) => issue.severity === "error").length} validation error(s)`}</li>
          <li>{lastPackage?.verified ? "Package verified · Ready" : "Package not built"}</li>
        </ul>
        {!nativeTransport && (
