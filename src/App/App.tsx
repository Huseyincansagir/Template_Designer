...
export function App({ profileRegistry }: { profileRegistry: DeviceProfileRegistry }) {
  ...
  const renderProperties = () => {
    ...
    const geometryEditable = canvasPointer.mode === "idle" && selectedSceneWidgetsForEdit.length > 0 && selectedSceneWidgetsForEdit.every((current) => !current.locked);
      ...
      <>
        ...
        {selection && node ? <div className="properties-scroll">
          ...
          {widget && <>
            ...
            <section className="property-section"><div className="property-section-title">Presentation</div><PropertyRow label="Bindings" value={String(widget.bindings.length)} /><button type="button" className="property-inline-action" onClick={() => setBindingModal({ widgetId: widget.id })}>Open Binding Editor</button></section>
